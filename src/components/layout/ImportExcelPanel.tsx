...
export default function ImportExcelPanel({ onImport, templateUploaded }: ImportExcelPanelProps) {
  ...
  const onDropExcel = useCallback(async (accepted: File[]) => {
    ...
    try {
      ...
      if (parsed.length === 0) throw new Error("Tidak ada data yang terbaca. Pastikan kolom A = nomor, B = judul (teks cover), C = spek, D = spek lengkap, E = nama produk Shopee.");
      setRows(parsed);
    } catch (err) {
  ...
  const handleImport = useCallback(() => {
    ...
    const productImages: ProductImage[] = validRows.map((m) => ({
      ...
      namaProduk: m.row.namaProduk || undefined,
      textLayer: {
        title: {
                    ...
                    <td className="px-4 py-3">
                      {m.file ? (
                        ...
                        <span className="inline-flex items-center gap-1 text-xs text-red-500">
                          <XCircle size={12} /> Foto {m.row.nomor} tidak ditemukan
                        </span>
                      )}
                    </td>
        ...
        <div className="rounded-xl border border-main-border bg-slate-50 p-4 text-xs text-slate-500 space-y-2">
          ...
          <div className="overflow-x-auto">
            <table className="text-xs border-collapse">
              <thead>
                <tr>
                  {["Kolom A", "Kolom B", "Kolom C", "Kolom D", "Kolom E"].map((h) => (
              ...
              <tbody>
                <tr>
                  {["Nomor urut (1, 2, 3…)", "Judul Produk (teks di cover)", "Spek Singkat", "Spek Lengkap (opsional)", "Nama Produk (untuk Excel Shopee)"].map((v, i) => (
                    <td key={i} className="px-3 py-1.5 bg-white border border-slate-200 text-slate-500 italic">{v}</td>
                  ))}
                </tr>
                <tr>
                  {["1", "Lenovo Legion 5", "Core i7, 16GB", "Intel Core i7-13700H | 16GB DDR5 | ...", "Laptop Gaming Lenovo Legion 5 i7-13700H 16GB 512GB RTX4060"].map((v, i) => (
                    <td key={i} className="px-3 py-1.5 bg-white border border-slate-200 text-slate-700">{v}</td>
                  ))}
                </tr>
                <tr>
                  {["2", "Logitech MX3", "Wireless, 4000 DPI", "", "Mouse Wireless Logitech MX Master 3S 4000 DPI"].map((v, i) => (
                    <td key={i} className="px-3 py-1.5 bg-white border border-slate-200 text-slate-700">{v}</td>
                  ))}
                </tr>
              </tbody>
          ...
          <p className="text-slate-400">
            Foto produk diberi nama sesuai nomor urut: <code className="bg-slate-200 px-1 rounded">1.jpg</code>, <code className="bg-slate-200 px-1 rounded">2.png</code>, <code className="bg-slate-200 px-1 rounded">3.avif</code>, dst. (ekstensi bebas, yang penting nama filenya = nomor urut). Format didukung: JPG, JPEG, PNG, WEBP, AVIF, GIF, BMP, TIFF, SVG, HEIC, HEIF, ICO, JXL.
          </p>
          <p className="text-slate-400">
            <strong>Kolom B</strong> dipakai untuk teks judul di cover foto. <strong>Kolom E</strong> dipakai untuk Nama Produk di Excel Mass Upload Shopee (boleh lebih panjang &amp; lengkap). Kalau kolom E kosong, Nama Produk Shopee otomatis pakai kolom B.
          </p>
        </div>
            ...
            <div className="min-w-0">
              {photoFiles.length > 0 ? (
                ...
                <>
                  ...
                  <p className="text-xs text-slate-400">Nama file: 1.jpg, 2.png, 3.avif, dst · JPG PNG WEBP AVIF GIF BMP TIFF SVG HEIC JXL</p>
                </>
              )}
            </div>
          </div>
